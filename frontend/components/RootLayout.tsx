...
const stages = [
  { name: "Plan", color: "bg-blue-500", glowColor: "shadow-blue-500/50", gradient: "from-blue-500 to-blue-600", paths: ["/home", "/explorer"] },
...
export function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  const currentStageIndex = stages.findIndex((stage) =>
    stage.paths.some((path) => pathname === path)
  );
            ...
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Label */}
                  <span className={`relative z-10 text-[10px] font-medium ${pathname === tab.path ? "text-blue-600 font-medium" : ""}transition-colors ${
                    isActive ? "text-gray-900" : "text-gray-500"
                  }`}>
                    {tab.name}
                  </span>
