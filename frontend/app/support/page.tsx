  ...
  const sendPipeline = async (overrides?: { audio?: File; text?: string }) => {
    ...
    try {
      ...
      const apiMessages = [
        ...
        { role: 'user' as const, content: inputText || (audio ? '(Voice message)' : currentVideo ? '(See video)' : currentPdfs.length ? '(See attached PDF)' : '(See image)') },
      ];
      ...
      <div className="flex flex-col max-w-4xl mx-auto w-full">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-2">
          ...
          <div className="flex gap-2">
            ...
            <input ref={fileInputRef} type="file" accept="image/*,video/mp4,video/webm,video/quicktime,video/mpeg,.mov,.mp4,.webm,.mpeg,application/pdf,.pdf" multiple={!attachedVideo} className="hidden" onChange={handleImageSelect} />
            ...
            <button type="submit" disabled={isLoading || (!text.trim() && attachedImages.length === 0 && attachedPdfs.length === 0 && !attachedVideo)} className="p-3 rounded-lg bg-orange-500 hover:bg-orange-400 disabled:opacity-50">
